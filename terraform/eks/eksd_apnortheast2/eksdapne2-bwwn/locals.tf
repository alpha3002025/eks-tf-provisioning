locals {
  ...
  deploy_ebs_csi_driver      = var.deploy_ebs_csi_driver     ## (1)
  deploy_pod_identity_agent  = var.deploy_pod_identity_agent ## (2)
  ...
  enabled_node_disk_gp3      = var.enabled_node_disk_gp3 ## (3)
  ...
  access_entries = {
    accountadmin = {
      ...
      principal_arn = "arn:aws:iam::${var.account_id.id}:user/overtake-devops-admin"
      policy_associations = {
        kubeadmin = {
